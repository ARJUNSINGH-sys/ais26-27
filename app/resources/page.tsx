...
const conceptsData: Concept[] = [
  {
    ...
    summary:
      "The computational bedrock of Transformer architectures, calculating contextual token-to-token relevance through Query, Key, and Value matrix projections.",
...
def scaled_dot_product_attention(Q, K, V):
  ...
  },
  {
    ...
    summary:
      "Discrete 2D cross-correlation sliding parameterized weight kernels across spatial feature maps to detect hierarchical edges, textures, and object motifs.",
...
features = conv(x)  # shape: [1, 64, 224, 224]`,
  ...
  },
  {
    ...
    summary:
      "Iterative parameter optimization traversing complex, non-convex multi-dimensional loss topologies via first-order partial derivatives and momentum.",
...
optimizer.step()`,
  ...
  },
  {
    ...
    summary:
      "Reversing a forward Gaussian noise corruption process through discrete Markovian transitions to synthesize high-dimensional tensors from latent random variables.",
...
def q_sample(x_start, t, noise=None):
  ...
  },
  {
    ...
    category: "TRANSFORMERS",
    summary:
      "Hyperparameter modulating probability entropy during autoregressive generation: low values yield deterministic greediness, while higher values expand entropy.",
...
def sample_with_temperature(logits, temperature=0.7):
  ...
  },
  {
    ...
    category: "OPTIMIZATION",
    summary:
      "Measuring semantic relatedness between dense representations in high-dimensional vector space using the normalized dot product angle.",
...
export default function ResourcesPage() {
  ...
  const currentConcept =
    conceptsData.find((c) => c.id === activeConceptId) || conceptsData[0];
  ...
  const attentionWeights = [
    ...
    [0.1, 0.55, 0.2, 0.1, 0.05],
    [0.15, 0.2, 0.4, 0.15, 0.1],
    [0.05, 0.1, 0.35, 0.35, 0.15],
    [0.08, 0.12, 0.25, 0.2, 0.35],
  ];

  // Convolution state
  const [kernelType, setKernelType] = useState<"edge" | "sharpen" | "gaussian">(
    "edge",
  );
  const kernels = {
    edge: [
  ...
  const expValues = baseLogits.map((l) =>
    Math.exp(l / Math.max(temperature, 0.05)),
  );
  ...
  return (
    <SmoothScroll>
      <div className="min-h-screen flex flex-col bg-[#EFECE6] text-[#1A1816]">
        <Nav />
        ...
        <main className="grow pt-28 sm:pt-36 pb-24">
          <div className="shell">
            ...
            <div className="flex items-center gap-3 mb-6">
              <Link
                href="/"
                className="text-[12px] font-mono uppercase text-[#75716B] hover:text-[#1A1816] transition-colors"
              >
                ← Back to Home
              </Link>
              <span className="text-[#D9D4CA]">/</span>
              <span className="text-[12px] font-mono uppercase font-bold text-[#1A1816]">
                AIS Resource Center
              </span>
            </div>

            {/* Header Title Section */}
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12 sm:mb-16 pb-12 border-b border-[#D9D4CA]">
              <div className="max-w-[28ch]">
                <span className="text-[11px] font-mono tracking-[0.16em] uppercase text-[#75716B] block mb-3">
                  BENNETT UNIVERSITY · OPEN ARCHIVE
                </span>
                <h1 className="text-[38px] sm:text-[54px] lg:text-[64px] font-black tracking-[-0.035em] leading-[1.05] text-[#1A1816]">
                  Learning Resource Center
                </h1>
              </div>

              <div className="max-w-[42ch]">
                <p className="text-[15px] sm:text-[16px] text-[#75716B] leading-[1.6] mb-4">
                  Interactive conceptual visualizers, mathematical formulations,
                  and battle-tested PyTorch implementations created by the AI
                  Society cohort.
                </p>
                <div className="flex items-center gap-3 text-[12px] font-mono text-[#1A1816]">
                ...
                <button
                  ...
                  className={`px-4 sm:px-5 py-2.5 rounded-full text-[12px] font-mono tracking-wider transition-all duration-200 cursor-pointer ${
                    activeCategory === cat
                      ...
                      : "bg-[#E3DFD7] text-[#75716B] hover:text-[#1A1816] border border-[#D9D4CA]"
                  }`}
                >
                  {cat}
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-4 flex flex-col gap-3">
                <span className="text-[11px] font-mono uppercase tracking-widest text-[#75716B] mb-1">
                  ...
                  <button
                    ...
                    className={`p-5 rounded-[22px] text-left transition-all duration-200 border cursor-pointer ${
                      activeConceptId === concept.id
                        ...
                        : "bg-white text-[#1A1816] border-[#D9D4CA] hover:border-[#1A1816]"
                    }`}
                  >
              ...
              <div className="lg:col-span-8 bg-white border border-[#D9D4CA] rounded-[32px] sm:rounded-[36px] p-6 sm:p-10 shadow-xs">
                {/* Visualizer Header */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#D9D4CA] mb-8">
                  <div>
                    <span className="text-[11px] font-mono tracking-widest uppercase text-[#75716B] block mb-1">
                      TOPIC SPECIFICATION
                    </span>
                    <h2 className="text-[24px] sm:text-[30px] font-extrabold text-[#1A1816] tracking-tight">
                      {currentConcept.name}
                    </h2>
                  </div>

                  {/* Toggle between Interactive Visualizer and PyTorch Code */}
                  <div className="flex items-center p-1 bg-[#E3DFD7] rounded-full self-start sm:self-center">
                    <button
                      type="button"
                ...
                <div className="bg-[#EFECE6] border border-[#D9D4CA] rounded-[20px] p-4 sm:p-5 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <span className="text-[10px] font-mono text-[#75716B] uppercase block mb-1">
                      Mathematical Governing Law
                    </span>
                    <code className="text-[13px] sm:text-[14px] font-mono font-bold text-[#1A1816]">
                ...
                {activeViewMode === "pytorch" && (
                  <div className="bg-[#1A1816] text-white p-6 sm:p-8 rounded-[24px] overflow-x-auto border border-[#2E2A26]">
                    <div className="flex items-center justify-between pb-4 mb-4 border-b border-[#2E2A26] text-[11px] font-mono text-[#A29C92]">
                  ...
                  <div>
                    ...
                    {activeConceptId === "attention" && (
                      <div className="space-y-8">
                        <div>
                          <h4 className="text-[13px] font-mono uppercase tracking-wider text-[#1A1816] font-bold mb-3">
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-full text-[13px] font-mono transition-all cursor-pointer ${
                                  selectedTokenIdx === idx
                                    ? "bg-[#1A1816] text-white font-bold"
                                    : "bg-[#EFECE6] text-[#1A1816] border border-[#D9D4CA] hover:border-[#1A1816]"
                                }`}
                              >
                                {token}
                              </button>
                        ...
                        <div className="bg-[#1A1816] text-white p-6 sm:p-8 rounded-[24px] border border-[#2E2A26]">
                          <div className="flex items-center justify-between mb-6 pb-3 border-b border-[#2E2A26]">
                            <span className="text-[12px] font-mono uppercase tracking-widest text-[#A29C92]">
                          ...
                          <div className="space-y-4">
                            {tokens.map((targetToken, idx) => {
                              const weight =
                                attentionWeights[selectedTokenIdx][idx];
                          ...
                          <div className="flex flex-wrap gap-2">
                            <button
                              ...
                              className={`px-4 py-2 rounded-full text-[13px] font-mono cursor-pointer ${
                                kernelType === "edge"
                                  ? "bg-[#1A1816] text-white font-bold"
                                  : "bg-[#EFECE6] border border-[#D9D4CA] text-[#1A1816]"
                              }`}
                            >
                              Laplacian Edge
                            </button>
                          ...
                          <div className="flex flex-col sm:flex-row items-center justify-center gap-8 py-4">
                            <div>
                              ...
                              <div className="grid grid-cols-3 gap-1.5 p-3 bg-white/5 rounded-[16px]">
                                {[
                                  140, 255, 120, 240, 255, 230, 110, 220, 100,
                                ].map((v, i) => (
                                  <div
                                    key={i}
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-3 gap-1.5 p-3 bg-white/10 rounded-[16px] border border-white/20">
                                {kernels[kernelType].flat().map((v, i) => (
                                  <div
                                    key={i}
                                    className="w-10 h-10 rounded-[6px] bg-white text-[#1A1816] flex items-center justify-center font-mono text-[11px] font-bold"
                                  >
                                    {v > 0 ? `+${v}` : v}
                                  </div>
                                ))}
                      ...
                      <div className="space-y-8">
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            <h4 className="text-[13px] font-mono uppercase tracking-wider text-[#1A1816] font-bold">
                              Adjust Learning Rate (η)
                            </h4>
                            <span className="font-mono text-[14px] font-bold px-3 py-1 bg-[#EFECE6] rounded-[8px] border border-[#D9D4CA]">
                        ...
                        <div className="bg-[#1A1816] text-white p-6 sm:p-8 rounded-[24px] border border-[#2E2A26]">
                          <div className="flex items-center justify-between mb-4 pb-3 border-b border-[#2E2A26] text-[12px] font-mono text-[#A29C92]">
                            ...
                            >
                              ...
                              {(() => {
                                const pts = [
                                  { x: 70, y: 40 },
                                  {
                                    x: 70 + 140 * learningRate,
                                    y: 40 + 50 * learningRate,
                                  },
                                  {
                                    x: 140 + 100 * (1 - learningRate * 0.5),
                                    y: 80 + 30 * learningRate,
                                  },
                                  {
                                    x: 210 + 30 * learningRate,
                                    y: 104 + 5 * learningRate,
                                  },
                                  { x: 250, y: 110 },
                                ];
                                const d = pts.reduce(
                    ...
                    {activeConceptId === "diffusion" && (
                      <div className="space-y-8">
                        ...
                        <div className="bg-[#1A1816] text-white p-8 rounded-[24px] border border-[#2E2A26] flex flex-col items-center justify-center">
                          <div
                          ...
                          <span className="text-[12px] font-mono text-[#A29C92] mt-4">
                            {timestep === 0
                              ? "Pure Reconstructed Latent x_0"
                              : `Noise Level: ${Math.round((timestep / 1000) * 100)}%`}
                          </span>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="space-y-8">
                        <div>
                          ...
                          <input
                            ...
                            value={vectorAngle}
                            onChange={(e) =>
                              setVectorAngle(parseInt(e.target.value))
                            }
                            className="w-full accent-[#1A1816] cursor-pointer"
                          />
                        </div>
                        ...
                        <div className="bg-[#1A1816] text-white p-8 rounded-[24px] border border-[#2E2A26] flex flex-col items-center justify-center">
                          <svg
                            viewBox="0 0 300 200"
                            className="w-64 h-48 overflow-visible"
                          >
                            {/* Origin */}
