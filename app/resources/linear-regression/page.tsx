      ...
      <main className="grow pt-28 sm:pt-36 pb-32 bg-[#EFECE6] text-[#1A1816] min-h-screen">
        <div className="shell max-w-5xl">
          ...
          <div className="flex items-center gap-2 text-[11px] font-mono tracking-[0.16em] uppercase text-[#75716B] mb-8">
            ...
            <span>/</span>
            <span className="text-[#DE5D35] font-semibold">Regression · Linear Regression</span>
          </div>
          ...
          <header className="mb-14 border-b border-[#1A1816]/15 pb-10">
            ...
            <p className="text-[16px] sm:text-[18px] text-[#75716B] max-w-2xl leading-[1.6]">
              Minimizing orthogonal Euclidean residuals in continuous parameter space. Compare closed-form normal equations with iterative gradient optimization.
            </p>
          </header>
          ...
          <section className="mb-16">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
              <div>
                <span className="text-[11px] font-mono uppercase tracking-widest text-[#75716B]">01 / Residual Geometry</span>
                <h2 className="text-[22px] font-bold text-[#1A1816] mt-0.5">Sum of Squared Errors (SSE)</h2>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-5 text-[14px] text-[#4A4742] leading-[1.7] space-y-4">
                <p>
                  Linear regression fits a straight line <MathTex math="\hat{y} = m x + c" /> by minimizing the vertical distance (residual <MathTex math="e_i = y_i - \hat{y}_i" />) between points and the hypothesis line:
                </p>

                <div className="p-3 bg-[#FAF9F5] border border-[#1A1816]/15 font-mono text-[13px]">
                  <MathTex math="\text{MSE}(m, c) = \frac{1}{N} \sum_{i=1}^N \left( y_i - (m x_i + c) \right)^2" block />
                </div>
                ...
                <div className="p-4 bg-[#FAF9F5] border border-[#1A1816]/15 space-y-4">
                  <div>
                    <div className="flex justify-between text-[11px] font-mono text-[#75716B] mb-1">
                      <span>SLOPE (m)</span>
                      <span className="font-bold text-[#DE5D35]">{slope.toFixed(2)}</span>
                    </div>
                    <input
                      type="range"
                      min="0.1"
                  ...
                  <div>
                    <div className="flex justify-between text-[11px] font-mono text-[#75716B] mb-1">
                      ...
                      <span className="font-bold text-[#1A1816]">{intercept.toFixed(1)}</span>
                    </div>
                    <input
                      type="range"
                      min="-10"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-[#FAF9F5] border border-[#1A1816]/15">
                    <span className="block text-[10px] font-mono text-[#75716B]">MEAN SQUARED ERROR</span>
                    <span className="text-[18px] font-mono font-bold text-[#DE5D35]">{metrics.mse.toFixed(2)}</span>
                  </div>
                  <div className="p-3 bg-[#FAF9F5] border border-[#1A1816]/15">
                    <span className="block text-[10px] font-mono text-[#75716B]">COEFFICIENT R²</span>
                    <span className="text-[18px] font-mono font-bold text-[#1A1816]">{metrics.rSquared.toFixed(3)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-7 border border-[#1A1816]/15 bg-[#FAF9F5] p-5">
                ...
                <div className="relative bg-[#EBF5FB]/30 border border-[#1A1816]/10 p-2 rounded-[2px] h-72 flex items-end">
                  <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible select-none">
                    {/* Grid */}
                    <line x1="5" y1="95" x2="95" y2="95" stroke="#1A1816" strokeWidth="0.6" />
                    <line x1="5" y1="5" x2="5" y2="95" stroke="#1A1816" strokeWidth="0.6" />
                ...
                <p className="text-[12px] font-mono text-[#75716B] mt-3 text-center">
                  Red dashed hairlines indicate prediction error <MathTex math="e_i" />. OLS seeks to minimize the sum of their squared lengths.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="mb-16 border-t border-[#1A1816]/15 pt-12">
            <div className="mb-6">
              <span className="text-[11px] font-mono uppercase tracking-widest text-[#75716B]">02 / Vector Formulation</span>
              <h2 className="text-[22px] font-bold text-[#1A1816] mt-0.5">The Normal Equations Matrix Solution</h2>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-[14px] text-[#4A4742] leading-[1.7]">
              <div className="space-y-4">
                <p>
                  Rather than performing numerical gradient steps, the global optimal parameters <MathTex math="\beta = [c, m]^T" /> can be solved in a single step using matrix projection:
                </p>
                <div className="p-4 bg-[#FAF9F5] border border-[#1A1816]/15">
                  <span className="block text-[11px] font-mono uppercase text-[#75716B] mb-2">Closed-Form Normal Equation</span>
                  <MathTex math="\mathbf{\beta}^* = (\mathbf{X}^T \mathbf{X})^{-1} \mathbf{X}^T \mathbf{y}" block />
                </div>
              </div>

              <div className="space-y-4">
                <p>
                  When dataset dimension <MathTex math="D" /> is massive, computing the matrix inverse <MathTex math="(\mathbf{X}^T \mathbf{X})^{-1}" /> requires <MathTex math="\mathcal{O}(D^3)" /> computational complexity. For high-dimensional regimes, Stochastic Gradient Descent (SGD) is preferred.
                </p>
                <div className="p-4 bg-[#FAF9F5] border border-[#1A1816]/15 text-[12px] font-mono">
                  <span className="text-[#DE5D35] font-bold">OPTIMAL VALUE FOR THIS DATASET:</span>
                  <div className="mt-1">
                    m* = {optimalOLS.mOpt.toFixed(3)}, c* = {optimalOLS.cOpt.toFixed(2)}
                  </div>
                </div>
              </div>
            </div>
